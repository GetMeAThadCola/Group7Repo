...
const NavBar = () => {
  ...
  const { isAuthenticated, logout } = useAuth(); // ✅ updated
        ...
        <Container className="navbar-container">
          ...
          <Navbar.Collapse id="basic-navbar-nav">
            <Nav className="justify-content-end flex-grow-1 pe-3">
              ...
              <Nav.Item>
                <Link to="/shop" className="navbar-link" onClick={() => setExpand(false)}>
                  <span className="nav-link-label">Shop</span>
                </Link>
              </Nav.Item>
              <Nav.Item>
                <Link to="/cart" className="navbar-link" onClick={() => setExpand(false)}>
                  <span className="nav-link-label">Cart</span>
                </Link>
              </Nav.Item>
              <Nav.Item>
                {isAuthenticated ? (
                  <Link to="/account" className="navbar-link" onClick={() => setExpand(false)}>
          ...
          <div className="media-cart">
            {isAuthenticated ? (
              <Link to="/account">
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="black" className="nav-icon" style={{ marginRight: "1rem" }}>
                  <path
                    fillRule="evenodd"
            ...
            <Link to="/cart" className="cart" data-num={cartList.length} aria-label="Go to Cart Page">
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="black" className="nav-icon">
